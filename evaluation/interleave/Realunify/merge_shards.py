...
def main():
    parser = argparse.ArgumentParser(description='Merge RealUnify shard jsonl files')
    parser.add_argument('--data_path', required=True, help='Original RealUnify jsonl for ordering and coverage check')
    parser.add_argument('--shard_dir', required=True, help='Directory containing realunify_results_shard_*.jsonl')
    parser.add_argument('--output_file', required=True, help='Merged output jsonl path')
    args = parser.parse_args()

    shard_paths = sorted(glob.glob(os.path.join(args.shard_dir, 'realunify_results_shard_*.jsonl')))
    if not shard_paths:
