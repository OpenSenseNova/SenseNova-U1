...
def saved_opening_result_is_complete(json_path, output_dir, expected_steps):
    try:
        ...
        for index in range(expected_steps):
            ...
            image_path = image_name if os.path.isabs(image_name) else os.path.join(output_dir, os.path.basename(image_name))
...
class NEOT2IInferenceEngine:
    ...
    def chat(self, prompt, input_images, cfg_scale=1.0, img_cfg_scale=1.0, timestep_shift=1.0, cfg_interval=(0, 1), image_size=(256, 256), num_steps=50, gt_text=None, gt_images=None, system_message='', max_new_tokens=4096, think_mode='no_think'):
        ...
        images= [(image.clamp(0,1).permute(0,2,3,1).cpu().numpy() * 255.0).round().astype(np.uint8) for image in images]
...
def run_opening_generation(args, engine, system_message):
    ...
    expected_steps_by_uid = {
        real_data["total_uid"]: len(io_data["output_text"])
        for real_data, io_data in zip(real_data_list, io_data_list)
    }
            ...
            try:
                ...
                while attempt < max_attempts:
                    ...
                    candidate_image_names = [
                        f"{data_uid}-o-{image_index}.jpg"
                        for image_index, _ in enumerate(images[:gt_out_step])
                    ]
                    text_steps = split_generated_text(text)
                    candidate_text, candidate_images = normalize_output_steps(text_steps, candidate_image_names, gt_out_step)
                    candidate_score = min(len(candidate_text), gt_out_step) + min(len(candidate_image_names), gt_out_step)
...
def run_annotation_config_generation(args, engine, system_message):
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_json = os.path.join(output_dir, os.path.basename(args.input_json_path)[:-5] + f'_step{args.step}_result.json')
    result_json = {}
        ...
        for sample_index, sample_data in enumerate(tqdm(data_list, desc=f"推理 {annotation_name}")):
            ...
            for pattern in think_modes:
                try:
                    ...
                    text, images = engine.chat(
                        prompt, input_images=input_images, cfg_scale=args.cfg_scale,
                        img_cfg_scale=args.img_cfg_scale, timestep_shift=args.timestep_shift,
                        cfg_interval=tuple(args.cfg_interval),
                        image_size=image_size_list, num_steps=args.num_steps,
                        gt_text=None, gt_images=None, system_message=system_message,
                        max_new_tokens=args.max_new_tokens, think_mode=pattern
                    )

                    print(f'pattern: {pattern}, text: {text}')
                    save_images = []
                    for i, image in enumerate(images):
                        image_save_path = os.path.join(image_save_dir, f"{annotation_name}_sample_{sample_index}_{pattern}_image_{i}.jpg")
...
def parse_args():
    ...
    parser.add_argument('--input_json_path', type=str, default=None, help='输入 JSON 配置文件路径；提供时默认走 annotation_config 模式')
    parser.add_argument('--mode', type=str, default='auto', choices=['auto', 'opening', 'annotation_config'], help='推理数据格式')
    ...
    parser.add_argument('--cfg_interval', type=float, nargs=2, default=[0, 1.0], help='CFG生效的时间步区间, e.g. --cfg_interval 0 1.0')
    ...
    parser.add_argument('--max_generation_pixels', type=int, default=DEFAULT_MAX_GENERATION_PIXELS, help='初始输出图片像素上限，默认约 2048x2048')
    parser.add_argument('--oom_retry_max_pixels', type=int, default=DEFAULT_OOM_RETRY_MAX_PIXELS, help='CUDA OOM 后降分辨率重试的像素上限；设为 0 可关闭')
    ...
    parser.add_argument('--think_mode', '--think_pattern', dest='think_modes', type=str, nargs='+', default=['think', 'no_think'], help='think模式列表；--think_pattern 为兼容旧命令的别名')
    parser.add_argument('--overwrite', action='store_true', help='覆盖已生成的 OpenING 样本')
    parser.add_argument('--num_shards', type=int, default=int(os.getenv('WORLD_SIZE', '1')), help='总分片数；多进程/多卡并行时设为总进程数')
    ...
    parser.add_argument('--retry_short_outputs', type=int, default=0, help='如果生成步数少于 benchmark 期望步数，额外重试次数')
    parser.add_argument('--opening_step_prompt_style', type=str, default='can_be', choices=['none', 'can_be', 'must_exact'], help='OpenING prompt 前的步数提示风格；can_be 对齐 gpt-dalle_generation.py')
    parser.add_argument('--enforce_expected_steps_prompt', action='store_true', help='兼容旧参数：等价于 --opening_step_prompt_style must_exact')
