...
# Large-model launcher for Unimmmu.
...
INFERENCE_MODE="${INFERENCE_MODE:-i2t}"
RESUME="${RESUME:-0}"
LIMIT="${LIMIT:-}"
...
OUTPUT_DIR="${OUTPUT_DIR:-<OUTPUT_ROOT>/unimmmu/${MODEL_NAME}_${STEP}_cfg_${img_cfg_scale//./}_${cfg_scale//./}_interval_${CFG_INTERVAL_TAG}_ts_${TIMESTEP_TAG}_norm_${cfg_norm}${TARGET_IMAGE_TAG}/${INFERENCE_MODE}}"
DATA_PATH="${DATA_PATH:-<DATA_ROOT>/unimmmu/vqa/unimmmu_direct.jsonl}"
LOG_DIR="${LOG_DIR:-${OUTPUT_DIR}/worker_logs}"
MERGED_OUTPUT="${MERGED_OUTPUT:-${OUTPUT_DIR}/unimmmu_results.jsonl}"
...
for shard_rank in "${!DEVICE_GROUPS[@]}"; do
  ...
  worker_log="$LOG_DIR/unimmmu_shard_${shard_rank}.log"
  ...
  (
    ...
    cmd=("$PYTHON_BIN" Unimmmu/inference_unimmmu.py
...
if (( failed != 0 )); then
  echo "At least one Unimmmu worker failed. Check logs under $LOG_DIR."
  exit 1
fi

python Unimmmu/merge_shards.py \
  --data_path "$DATA_PATH" \
...
echo "Unimmmu large-model run finished."
