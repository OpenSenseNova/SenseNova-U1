...
from sensenovalm.core.scheduler import comm
...
class PipelineScheduler(BaseScheduler):
    ...
    def load_batch(self, engine, data_iter):
        ...
        # because sensenovalm's current train dataset is packed, even using dummy data.
    ...
    def _forward_step(
        ...
        accum_moe_coef_loss=None,
        accum_extra_loss=None,
    ):
        """
        ...
        _extra_loss = None  # boi loss

        self._call_hooks("before_forward", data)
        *output_obj, boi_loss, moe_losses, moe_z_losses, moe_coef_losses = self._call_engine(engine.model, data)
        ...
        if gpc.is_last_rank(ParallelMode.PIPELINE):
            ...
            if accum_loss is not None:
                ...
                extra_losses, _extra_loss = [], 0.0
                if boi_loss is not None:
                    if isinstance(boi_loss, list):
                        assert all(isinstance(x, (int, float, torch.Tensor)) for x in boi_loss)
                        other_loss_sum = sum(boi_loss) / self.num_microbatches
                    elif isinstance(boi_loss, (int, float, torch.Tensor)):
                        other_loss_sum = boi_loss / self.num_microbatches
                    else:
        ...
        if moe_losses is not None and len(moe_losses) > 0:
            assert all(isinstance(x, (int, float, torch.Tensor)) for x in moe_losses)
            moe_loss = sum(moe_losses)
            moe_loss /= self.num_microbatches
            accum_moe_loss.add_(moe_loss.detach())
        if moe_z_losses is not None and len(moe_z_losses) > 0:
            assert all(isinstance(x, (int, float, torch.Tensor)) for x in moe_z_losses)
            moe_z_loss = sum(moe_z_losses)
            moe_z_loss /= self.num_microbatches
            accum_moe_z_loss.add_(moe_z_loss.detach())
        if moe_coef_losses is not None and len(moe_coef_losses) > 0:
            assert all(isinstance(x, (int, float, torch.Tensor)) for x in moe_coef_losses)
            moe_coef_loss = sum(moe_coef_losses)
            moe_coef_loss /= self.num_microbatches
            accum_moe_coef_loss.add_(moe_coef_loss.detach())

        if accum_extra_loss is not None and boi_loss is not None:
            accum_extra_loss.add_(_extra_loss.detach())

        return output_obj, _extra_loss, moe_loss, moe_z_loss, moe_coef_loss
    ...
    def _backward_step(
        ...
        if input_obj is not None:
            if isinstance(input_obj, torch.Tensor) and input_obj.requires_grad is True:
                input_obj.retain_grad()
            else:
                for idx, in_tensor in enumerate(input_obj):
                    if in_tensor is not None and in_tensor.requires_grad is True:
        ...
        # filter obj with requires_grad False
        if not isinstance(output_obj, torch.Tensor):
            output_obj_requires_grad_true = []
            output_obj_grad_requires_grad_true = None
            if output_obj_grad is not None:
                output_obj_grad_requires_grad_true = []
            for idx, obj in enumerate(output_obj):
                if obj.requires_grad is True:
                    output_obj_requires_grad_true.append(obj)
                    if output_obj_grad is not None:
                        output_obj_grad_requires_grad_true.append(output_obj_grad[idx])

            output_obj = output_obj_requires_grad_true
            output_obj_grad = output_obj_grad_requires_grad_true
        ...
        with switch_optimizer_grad_sync_skip_mode(engine.optimizer, skip_grad_sync):
            if moe_loss is None or moe_loss.item() == 0.0:
                if output_obj_grad is None and gpc.is_last_rank(ParallelMode.PIPELINE):
                    engine.backward(output_obj + extra_loss)
                else:
                    engine.backward_by_grad(output_obj, output_obj_grad)
            else:
                if moe_z_loss is not None and moe_z_loss.item() != 0.0:
                    moe_loss = moe_loss + moe_z_loss
                if moe_coef_loss is not None and moe_coef_loss.item() != 0.0:
                    moe_loss = moe_loss + moe_coef_loss
                if output_obj_grad is None and gpc.is_last_rank(ParallelMode.PIPELINE):
                    engine.backward(output_obj + extra_loss + moe_loss)
                else:
                    ...
                    if isinstance(output_obj, torch.Tensor):
                        ...
                        assert isinstance(output_obj, (tuple, list))
                        engine.backward_by_grad([*output_obj, moe_loss], [*output_obj_grad, None])
    ...
    def _forward_only_step(self, engine, return_loss=True, return_output_label=True):
        ...
        Args:
            engine (colossalai.engine.Engine): sensenovalm engine for training and inference.
        ...
        """
        assert False, "forward only is not supported for multimodal"
    ...
    def _forward_backward_step(self, engine, return_loss=True, return_output_label=True):
        ...
        input_objs = []
        output_objs = []
        backward_recv_shapes_list = []
        backward_recv_dtypes_list = []
        moe_losses = []
        moe_z_losses = []
        moe_coef_losses = []
        extra_losses = []
        ...
        accum_moe_coef_loss = torch.zeros(1, device=get_current_device())
        accum_extra_loss = (
            torch.zeros(1, device=get_current_device())
            if return_loss
            and gpc.is_pipeline_last_stage(ignore_virtual=True)
            else None
        )

        # Used for tensor meta information communication
        forward_recv_shapes = self.tensor_shape
        forward_recv_dtypes = self.dtype
        forward_recv_requires_grads = None
        backward_recv_shapes = None
        backward_recv_dtypes = None
        need_forward_meta = self.tensor_shape is None
        ...
        for i in range(num_warmup_microsteps):
            ...
            if not gpc.is_first_rank(ParallelMode.PIPELINE):
                # if forward_recv_shapes is None:
                if need_forward_meta:
                    forward_recv_shapes, forward_recv_dtypes, forward_recv_requires_grads = comm.recv_obj_meta()
                input_obj = comm.recv_forward(
                    forward_recv_shapes,
                    dtypes=forward_recv_dtypes,
                    requires_grads=forward_recv_requires_grads,
                    scatter_gather_tensors=self.scatter_gather_tensors,
                )
            else:
                input_obj = None
            ...
            # Perform forward computation
            output_obj, extra_loss, moe_loss, moe_z_loss, moe_coef_loss = self._forward_step(
                engine,
                input_obj,
                return_tensors,
                return_output_label=return_output_label,
                ...
                accum_moe_coef_loss=accum_moe_coef_loss,
                accum_extra_loss=accum_extra_loss,
            )

            if not gpc.is_last_rank(ParallelMode.PIPELINE):
                if isinstance(output_obj, torch.Tensor):
                    backward_recv_shapes = output_obj.shape if output_obj.requires_grad else torch.Size(())
                    backward_recv_dtypes = output_obj.dtype if output_obj.requires_grad else torch.int64
                else:
                    backward_recv_shapes = [
                        out_tensor.shape if out_tensor.requires_grad else torch.Size(()) for out_tensor in output_obj
                    ]
                    backward_recv_dtypes = [
                        out_tensor.dtype if out_tensor.requires_grad else torch.int64 for out_tensor in output_obj
                    ]

                backward_recv_shapes_list.append(backward_recv_shapes)
                backward_recv_dtypes_list.append(backward_recv_dtypes)
            ...
            moe_coef_losses.append(moe_coef_loss)
        ...
        for i in range(num_1f1b_micropairs):
            ...
            if gpc.is_last_rank(ParallelMode.PIPELINE):
                ...
                backward_recv_shapes = backward_recv_shapes_list.pop(0)
                backward_recv_dtypes = backward_recv_dtypes_list.pop(0)

                output_obj_grad = comm.send_forward_recv_backward(
                    output_obj,
                    backward_recv_shapes,
                    dtypes=backward_recv_dtypes,
                    scatter_gather_tensors=self.scatter_gather_tensors,
                )
            ...
            moe_coef_loss = moe_coef_losses.pop(0)
            extra_loss = extra_losses.pop(0)

            input_obj_grad = self._backward_step(
                engine, i, input_obj, output_obj, output_obj_grad, extra_loss, moe_loss, moe_z_loss, moe_coef_loss
            )
        ...
        if gpc.config.model.use_moe:
            dist.all_reduce(accum_moe_loss, group=gpc.get_group(ParallelMode.PIPELINE))
            dist.all_reduce(accum_moe_z_loss, group=gpc.get_group(ParallelMode.PIPELINE))
            dist.all_reduce(accum_moe_coef_loss, group=gpc.get_group(ParallelMode.PIPELINE))

        return output, label, accum_loss, accum_extra_loss, accum_moe_loss, accum_moe_z_loss, accum_moe_coef_loss
    ...
    def forward_backward_step(self, engine, data_iter, forward_only=False, return_loss=True, return_output_label=True):
        ...
        if forward_only:
            output, label, accum_loss, *_extra_losses = self._forward_only_step(
                engine, return_loss, return_output_label
            )
        else:
            output, label, accum_loss, *_extra_losses = self._forward_backward_step(
                engine, return_loss, return_output_label
            )
        ...
        if getattr(gpc.config.model.moe_layer_kwargs, "aux_free", False):
            for layer in _submodule_filter(engine.model, SenseNovaVLMoE):
                ...
                if not hasattr(layer.moe_layer, "e_score_correction_bias_fp32"):
                    layer.moe_layer.e_score_correction_bias_fp32 = layer.moe_layer.e_score_correction_bias.to(torch.float32)
                expert_biases_list.append(layer.moe_layer.e_score_correction_bias_fp32)
            ...
            for layer in _submodule_filter(engine.model, SenseNovaVLMoE):
                ...
                layer.moe_layer.e_score_correction_bias_fp32.data.copy_(expert_biases_list[moe_layer_id])
                torch.zero_(layer.moe_layer.expert_counts)
                moe_layer_id += 1
        
        extra_losses = []
        if _extra_losses:
            for _extra_loss in _extra_losses:
                if _extra_loss is not None and _extra_loss.item() != 0.0:
                    extra_losses.append(_extra_loss)

        return output, label, accum_loss, *extra_losses
...
class InterleavedPipelineScheduler(PipelineScheduler):
    ...
    def __init__(
        ...
        """
        assert False, "interleaved pipeline scheduler is not supported for multimodal"
