...
def concat_pad_data_collator(features, max_item_length=None, pad_id=0):
    ...
    for idx in range(len(features)):
        ...
        if "type_ids" in feat:
            temp_type_ids = torch.LongTensor([pad_id] * max_item_length)
            temp_type_ids[: feat["type_ids"].shape[0]] = feat["type_ids"]
            feat["type_ids"] = temp_type_ids
    ...
    for k, v in first.items():
        ...
        # TODO, TBD
        #  
        # if k in ("pixel_values", "image_flags", "image_con_flags"):
        #     if isinstance(v, torch.Tensor):
        ...
        if k in ('pixel_values', 'image_seq_lens', 'image_flags', 'image_con_flags', 'image_for_gen_flags', 'image_for_gen_loss_flags', 'is_image_duplicated_for_und_flags'):
            ...
            if len(valid_items) == 0:
                ...
                batch[k] = [item for _ in valid_items for item in _]
            else:
                # TODO: TBD
                #  batch[k] = torch.concat([f[k] for f in features])
                batch[k] = torch.concat(valid_items)
