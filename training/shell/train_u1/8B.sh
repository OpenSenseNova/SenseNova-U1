#!/bin/bash
# Standalone torchrun launcher for SenseNova-U1 8B (dense + MoT image-gen).
#
# Single-node (8 GPUs):
#   bash shell/train_u1/8B.sh
#
# Multi-node — run on each node, replacing NODE_RANK / MASTER_ADDR:
#   NNODES=2 NODE_RANK=0 MASTER_ADDR=10.0.0.1 bash shell/train_u1/8B.sh
#   NNODES=2 NODE_RANK=1 MASTER_ADDR=10.0.0.1 bash shell/train_u1/8B.sh
...
# ============================ Job / logging ============================ #
export JOB_NAME=${JOB_NAME:-"sensenovau1_8b_smoke_test"}
